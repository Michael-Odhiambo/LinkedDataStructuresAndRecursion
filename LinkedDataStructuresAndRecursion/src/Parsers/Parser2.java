package Parsers;
/**
 * This program evaluates standard expression typed in by the user. The expressions
 * can use positive real numbers and the binary operators +, -, *, and /. The unary
 * minus operation is supported. The expressions are defined by the BNF rules:
 *
 *           <expression>  ::=  [ "-" ] <term> [ [ "+" | "-" ] <term> ] ...
 *           <term>  ::=  <factor> [ [ "*" | "/" ] <factor> ] ...
 *           <factor>  ::=  <number>  |  "(" <expression> ")"
 *
 * A number must begin with a digit ( i.e., not a decimal point ). A line of input
 * must contain exactly one such expression. If extra data is found on a line after
 * an expression has been read, it is considered an error.
 */
...
public class Parser2 {
    ...
    private static double termValue() throws ParseError {
        ...
        while ( TextIO.peek() == '*' || TextIO.peek() == '/' ) {
            // Read the next factor, and multiply or divide the value-so-far
            // by the value of this factor.
            char op = TextIO.getAnyChar();
